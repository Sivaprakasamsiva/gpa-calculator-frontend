...
const GPACalculator = () => {
  ...
  // original subjects from API for current semester
  ...
  // LOCAL loading only (very important – no useAuth)
  ...
  const buildRowsFromSubjects = (subjectList) => {
    ...
    const mandatoryRows = mandatory.map((s) => ({
      ...
      credits: s.credits,
      type: s.type || "CORE",
      grade: "", // user will choose
    }));

    // Electives: slot rows – user will pick the subject
  ...
  const handleCalculateGPA = async () => {
    ...
    const gradedRows = rows.filter(
      (r) =>
        r.subjectId &&
        r.grade &&
        r.grade !== "NOT_HAVE" // ignore special value if you use it
    );
