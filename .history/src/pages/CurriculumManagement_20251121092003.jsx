    ...
    <div className={darkMode ? "dark min-h-screen" : "min-h-screen"}>
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            {/* Regulation */}
            <div>
              <label className="text-sm dark:text-white">Regulation</label>
              <div className="flex">
                <select
            ...
            {/* Department */}
            <div>
              <label className="text-sm dark:text-white">Department</label>
              <div className="flex">
                <select
                  className="w-full p-3 border rounded-lg dark:bg-gray-800 dark:text-white"
                  value={deptId}
            ...
            {/* Semester */}
            <div>
              <label className="text-sm dark:text-white">Semester</label>
              <div className="flex">
                <select
                  className="w-full p-3 border rounded-lg dark:bg-gray-800 dark:bg-gray-800"
                  value={semesterId}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="border rounded-lg dark:bg-gray-800">
              <div className="p-4 border-b">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-300">{filteredMandatory.length} items</p>
              </div>
                  ...
                  {loadingSubjects ? (
                    ...
                    filteredMandatory.map(sb => (
                      <tr key={sb.id} className="border-b">
                        <td className="p-3 dark:text-white">{sb.code}</td>
            ...
            <div className="border rounded-lg dark:bg-gray-800">
              <div className="p-4 border-b">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-300">{filteredElective.length} items</p>
              </div>
      ...
      {showRegModal && (
        <Modal onClose={() => setShowRegModal(false)}>
          <h2 className="text-xl mb-3 dark:text-white">{regForm.id ? "Edit Regulation" : "Add Regulation"}</h2>
          <form onSubmit={submitReg} className="space-y-3">
            <input value={regForm.name} onChange={(e) => setRegForm({ ...regForm, name: e.target.value })} placeholder="Name" className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white" />
            <input type="number" value={regForm.year} onChange={(e) => setRegForm({ ...regForm, year: Number(e.target.value) })} placeholder="Year" className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white" />
            <textarea value={regForm.description} onChange={(e) => setRegForm({ ...regForm, description: e.target.value })} placeholder="Description" className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white" />
            <div className="flex justify-end space-x-2">
              <button type="button" onClick={() => setShowRegModal(false)} className="px-3 py-2 bg-gray-300 rounded dark:text-black">Cancel</button>
      ...
      {showDeptModal && (
        <Modal onClose={() => setShowDeptModal(false)}>
          <h2 className="text-xl mb-3 dark:text-white">{deptForm.id ? "Edit Department" : "Add Department"}</h2>
          <form onSubmit={submitDept} className="space-y-3">
            <select value={deptForm.regulationId} onChange={(e) => setDeptForm({ ...deptForm, regulationId: e.target.value })} className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white">
              <option value="">Select Regulation</option>
              {regulations.map(r => <option key={r.id} value={r.id}>{r.name} - {r.year}</option>)}
            </select>
            <input value={deptForm.code} onChange={(e) => setDeptForm({ ...deptForm, code: e.target.value })} placeholder="Code" className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white" />
            <input value={deptForm.name} onChange={(e) => setDeptForm({ ...deptForm, name: e.target.value })} placeholder="Name" className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white" />
            <input type="number" value={deptForm.semesterCount} onChange={(e) => setDeptForm({ ...deptForm, semesterCount: e.target.value })} placeholder="Semesters (default 8)" className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white" />
            <textarea value={deptForm.description} onChange={(e) => setDeptForm({ ...deptForm, description: e.target.value })} placeholder="Description" className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white" />
            <div className="flex justify-end space-x-2">
              <button type="button" onClick={() => setShowDeptModal(false)} className="px-3 py-2 bg-gray-300 rounded dark:text-black">Cancel</button>
              <button type="submit" className="px-3 py-2 bg-blue-600 text-white rounded">Save</button>
            </div>
          </form>
        </Modal>
      ...
      {showSemModal && (
        <Modal onClose={() => setShowSemModal(false)}>
          <h2 className="text-xl mb-3 dark:text-white">{semForm.id ? "Edit Semester" : "Add Semester"}</h2>
          <form onSubmit={submitSem} className="space-y-3">
            <select value={semForm.regulationId} onChange={(e) => setSemForm({ ...semForm, regulationId: e.target.value })} className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white">
              <option value="">Select Regulation</option>
              {regulations.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
            </select>

            <select value={semForm.departmentId} onChange={(e) => setSemForm({ ...semForm, departmentId: e.target.value })} className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white">
              <option value="">Select Department</option>
              {departments.map(d => <option key={d.id} value={d.id}>{d.code} - {d.name}</option>)}
            </select>

            <input type="number" min="1" value={semForm.number} onChange={(e) => setSemForm({ ...semForm, number: e.target.value })} className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white" placeholder="Semester number" />

            <div className="grid grid-cols-2 gap-2">
              <input type="number" min="0" value={semForm.mandatoryCount} onChange={(e) => setSemForm({ ...semForm, mandatoryCount: e.target.value })} className="p-2 border rounded dark:bg-gray-800 dark:text-white" placeholder="Mandatory" />
              <input type="number" min="0" value={semForm.electiveCount} onChange={(e) => setSemForm({ ...semForm, electiveCount: e.target.value })} className="p-2 border rounded dark:bg-gray-800 dark:text-white" placeholder="Elective" />
            </div>

            <textarea value={semForm.description} onChange={(e) => setSemForm({ ...semForm, description: e.target.value })} placeholder="Description" className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white" />

            <div className="flex justify-end space-x-2">
              <button type="button" onClick={() => setShowSemModal(false)} className="px-3 py-2 bg-gray-300 rounded dark:text-black">Cancel</button>
              <button type="submit" className="px-3 py-2 bg-blue-600 text-white rounded">Save</button>
            </div>
          </form>
        </Modal>
      ...
      {showSubjectModal && (
        <Modal onClose={() => setShowSubjectModal(false)}>
          <h2 className="text-xl mb-3 dark:text-white">{subjectForm.id ? "Edit Subject" : "Add Subject"}</h2>
          <form onSubmit={submitSubject} className="space-y-3">
            <input value={subjectForm.code} onChange={(e) => setSubjectForm({ ...subjectForm, code: e.target.value })} placeholder="Code" className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white" />
            <input value={subjectForm.name} onChange={(e) => setSubjectForm({ ...subjectForm, name: e.target.value })} placeholder="Name" className="w-full p-2 border rounded dark:bg-gray-800 dark:text-white" />

            <div className="grid grid-cols-3 gap-2">
              <input type="number" value={subjectForm.credits} onChange={(e) => setSubjectForm({ ...subjectForm, credits: e.target.value })} className="p-2 border rounded dark:bg-gray-800 dark:text-white" />
              <select value={subjectForm.type} onChange={(e) => setSubjectForm({ ...subjectForm, type: e.target.value })} className="p-2 border rounded dark:bg-gray-800 dark:text-white">
            ...
            <div className="flex justify-end space-x-2">
              <button type="button" onClick={() => setShowSubjectModal(false)} className="px-3 py-2 bg-gray-300 rounded dark:text-black">Cancel</button>
              <button type="submit" className="px-3 py-2 bg-blue-600 text-white rounded">Save</button>
            </div>
          </form>
        </Modal>
        ...
        <Modal onClose={() => { setShowSqlModal(false); resetBulkModal(); }}>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-xl dark:text-white">Bulk Insert Data</h2>
              <div className="flex items-center space-x-2">
                <label className="text-sm dark:text-white">Type</label>
                <select value={bulkType} onChange={(e) => setBulkType(e.target.value)} className="p-2 border rounded dark:bg-gray-800 dark:text-white">
            ...
            <div className="flex gap-2">
              <button className={`px-3 py-1 rounded ${bulkTab === "sql" ? "bg-blue-600 text-white" : "bg-gray-200 dark:bg-gray-700 dark:text-white"}`} onClick={() => setBulkTab("sql")}>SQL → JSON</button>
              <button className={`px-3 py-1 rounded ${bulkTab === "csv" ? "bg-blue-600 text-white" : "bg-gray-200 dark:bg-gray-700 dark:text-white"}`} onClick={() => setBulkTab("csv")}>CSV</button>
              <button className={`px-3 py-1 rounded ${bulkTab === "json" ? "bg-blue-600 text-white" : "bg-gray-200 dark:bg-gray-700 dark:text-white"}`} onClick={() => setBulkTab("json")}>JSON</button>
              <div className="ml-auto text-sm text-gray-500 dark:text-gray-300">Pro tip: set Regulation & Department filters first</div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => { setBulkText(""); setBulkPreview([]); }} className="px-4 py-2 bg-gray-300 rounded dark:text-black">Clear</button>
              <div className="ml-auto text-sm text-gray-500 dark:text-gray-300">{bulkPreview.length} rows parsed</div>
            </div>

            {/* PREVIEW */}
            <div className="max-h-64 overflow-auto border rounded p-2 bg-white dark:bg-gray-800">
              {bulkPreview.length === 0 ? (
                <div className="text-sm text-gray-500 p-4 dark:text-gray-300">Preview will appear here after parsing.</div>
              ) : (
                <table className="w-full text-sm table-auto">
                  <thead>
                    <tr>
                      {Object.keys(bulkPreview[0]).map((k) => (
                        <th key={k} className="p-2 text-left text-xs text-gray-600 dark:text-gray-300">{k}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
            ...
            <div className="flex justify-end space-x-2">
              <button onClick={() => { setShowSqlModal(false); resetBulkModal(); }} className="px-3 py-2 bg-gray-300 rounded dark:text-black">Cancel</button>
...
const Modal = ({ children, onClose }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4">
    <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-3xl relative">
      <button onClick={onClose} className="absolute top-3 right-3 px-2 py-1 text-sm dark:text-white">✕</button>
      {children}
    </div>
  </div>
);
