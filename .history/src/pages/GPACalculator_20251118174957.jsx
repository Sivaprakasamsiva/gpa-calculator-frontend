...
const GPACalculator = () => {
  ...
  const [rawSubjects, setRawSubjects] = useState([]); // original from API

  // rows shown in table (mandatory + elective placeholders)
  ...
  // --------------------------------------------------
  // Helper: build rows when subjects loaded
  // --------------------------------------------------
  const buildRowsFromSubjects = (subjectList) => {
    ...
    // Mandatory rows: fixed subject per row
    const mandatoryRows = mandatory.map((s) => ({
      ...
      credits: s.credits,
      type: s.type || "CORE",
      grade: "", // user will pick
    }));

    // Elective rows: placeholder rows; subject chosen from dropdown
    const electiveRows = electives.map((_, index) => ({
      rowId: `ELEC-${index + 1}`,
      mode: "elective",
      isElective: true,
      slotNumber: index + 1,
      subjectId: "", // not selected yet
      code: "",
      name: "",
      credits: null,
      type: "ELECTIVE",
  ...
  const handleSemesterChange = async (value) => {
    ...
    try {
      ...
      setRawSubjects(list);
      const built = buildRowsFromSubjects(list);
      setRows(built);

      if (list.length === 0) {
        toast.info("No subjects configured for this semester.");
      } else {
        console.log("📌 SUBJECTS LOADED INTO UI:", built);
      }
    } catch (err) {
  ...
  // --------------------------------------------------
  // Elective subject selection change
  // --------------------------------------------------
      ...
      prev.map((row) => {
        ...
        if (!subjectId) {
          ...
          return {
            ...
            name: "",
            credits: null,
            // keep grade or clear? safer to clear
            grade: "",
          };
        }
  ...
  const handleCalculateGPA = async () => {
    ...
    // only rows with a real grade and a chosen subject
    const gradedRows = rows.filter(
      (r) =>
        r.subjectId &&
        r.grade &&
        r.grade !== "NOT_HAVE" // ignore "Not Have"
    );
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden mb-8">
                <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    ...
                    the elective paper and grade. Use{" "}
                    <strong>Not Have (Ignore)</strong> for subjects you do not
                    have.
                  </p>
                </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
              <div className="flex flex-wrap gap-4">
                <button
                  ...
                  disabled={!semester || rows.length === 0}
