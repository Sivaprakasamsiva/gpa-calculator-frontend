...
const GPACalculator = () => {
  ...
  const [rawSubjects, setRawSubjects] = useState([]); // all subjects for that sem
  const [rows, setRows] = useState([]); // UI rows (mandatory + elective slots)
  ...
  // -----------------------------
  // Build rows from subjects + electiveCount
  // -----------------------------
  const buildRowsFromSubjects = (subjectList, electiveCount) => {
    ...
    // Elective rows – number of slots = electiveCount from DB
  ...
  const handleSemesterChange = async (value) => {
    ...
    try {
      ...
      // 🔄 Load subjects + semester details in parallel
